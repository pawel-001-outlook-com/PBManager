...
namespace PBManager.Web.Controllers
{
    public class DashboardMainController : BaseController
    {
        ...
        public ActionResult GetExpensesData(ReportFormDataRequestDto reportFormDataRequestDto)
        {
            ...
            if (z.Count > 0)
            {
                DateTime cdate = z.Count() > 0 ? z.FirstOrDefault().date : default(DateTime);
                DateTime endDate = z.Count() > 0 ? z.LastOrDefault().date : default(DateTime);
                int i = 0;
            ...
            {
                int year = Convert.ToInt32(reportFormDataRequestDto.month.Substring(0, 4));
                int month = Convert.ToInt32(reportFormDataRequestDto.month.Substring(5, 2));
                var DaysInMonth = DateTime.DaysInMonth(year, month);


                DateTime cdate = new DateTime(year, month, 1);
