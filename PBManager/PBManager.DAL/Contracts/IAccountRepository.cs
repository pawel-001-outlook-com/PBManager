...
namespace PBManager.DAL.Contracts
{
    public interface IAccountRepository
    {
        ...
        void Update(IEnumerable<Account> accounts);
        void Remove(Account account);
        void Add(Account account);
        void Delete(Account account);
        int GetTotalCount();
        int GetFilteredCount(string searchValue);
        List<Account> GetDataFilteredSorted(string searchValue, string sortDirection, string sortColumnName, int start, int length);
    }
}